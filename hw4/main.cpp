...
void DMAInit()
{
	...
	LPC_GPDMACH0->DMACCDestAddr = (uint32_t) &(LPC_GPIO2->FIOPIN0); // LED is connected to P1.29.
        // the array where alternating 1's and 0's. are stored
        LPC_GPDMACH0->DMACCSrcAddr = (uint32_t) &sample0[0]; 
	LPC_GPDMACH0->DMACCLLI = 0;
	
	LPC_GPDMACH0->DMACCControl = 1024 | (6 << 12) |  (6 << 15) |( 1 << 26 ); // Transfer size = 200 bytes and enable source increment.
